...
(function ($) {
    ...
    var PageGrid = {
        ...
        renderData: function (R, $grid, pageProps) {
            ...
            var attr = new Array();
            //row click
            var rowfunc = pageProps.rowfunc||"";
            for (var i = 0; i < data.length; i++) {
                //原始数据
                var rowdata=$.extend(true,[], data[i]);
                //为数据创建tr
                if(rowfunc){
                    var func = eval((rowfunc.func));
                    var prop = eval((rowfunc));
                    $(_grid.find("tbody")).append(func(rowdata,prop));
                }else{
                ...
                for (var j = 0; j < _th.length; j++) {
                    ...
                    if (operate == "true") {
                       ...
                       // var btns = buttons.split(",");
                        var btns = eval(buttons); //这是一个数组，每项为json串
                        var tdContent = "<span></span>"
                        for (var k = 0; k < btns.length; k++) {
                            var func = eval((btns[k].func));
                            var prop = eval((btns[k]));
                            tdContent = tdContent.replace("</span>", func(rowdata,prop) + "</span>");
                        }
