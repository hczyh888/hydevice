...
(function ($) {
    ...
    $.fn.treeTool = function () {
        ...
        $(this).attr("style", "padding-right: 20px;");
        //为该组件添加清空按钮
        ...
        $(this).after('<input value="' + _value + '"  style="display: none" id="' + _id + '_id"  name="' + cyProps.name + '"  class="layui-input">' +
            ...
            '<div ><input type="text" placeholder="搜索" value="" id="keyword" class="layui-input "></div>' +
...
function openZtree(obj) {
    ...
    $.ajax({
        ...
        success: function (R) {
            if (R.code == 0) {
                ztree = $.fn.zTree.init($("#zTree"), setting, R.data);
                fuzzySearch('zTree','#keyword',false,true); //初始化模糊搜索方法
                var node = ztree.getNodeByParam("id", _value);
                if (node != null) {
                    ztree.selectNode(node);
                    $(obj).val(node.name);
