...
var Render = {
     ...
     * @param rowdata    行数据
     * @param renderData 渲染后的列
     * @description      导出申请表按钮渲染
     */
    export:function(rowdata,renderData){
        var btn=' <button  onclick="detailOne(\''+"/sysoss/info"+'\',\''+rowdata.bucket+'\')" class="layui-btn layui-btn-mini">导出申请表</button>';
        return btn;
    },
    /**
     * @param rowdata    行数据
    ...
    edit:function(rowdata,renderData){
        var btn=' <button  onclick="editOne(\''+"/plan/planApply/edit"+'\',\''+rowdata.id+'\')" class="layui-btn layui-btn-sm">修改</button>';
        return btn;
    },
    /**
     * @param rowdata    行数据
    ...
    delete:function(rowdata,renderData){
        var btn=' <button  onclick="deleteOne(\''+"删除"+'\',\''+"/plan/planApply/del"+'\',\''+rowdata.id+'\')" class="layui-btn layui-btn-sm layui-btn-delete">删除</button>';
        return btn;
    },
    /**
     * @param rowdata    行数据
