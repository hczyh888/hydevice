...
var Render = {
    ...
    edit:function(rowdata,renderData){
        var btn=' <button  onclick="editOne(\''+"/sys/menu/edit"+'\',\''+rowdata.id+'\')" class="layui-btn layui-btn-sm">修改</button>';
        return btn;
    },
    /**
     * @param rowdata    行数据
    ...
    delete:function(rowdata,renderData){
        var btn=' <button  onclick="deleteOne(\''+"删除"+'\',\''+"/sys/menu/del"+'\',\''+rowdata.id+'\')" class="layui-btn layui-btn-sm layui-btn-delete">删除</button>';
        return btn;
    },
    /**
     * @param rowdata    行数据
