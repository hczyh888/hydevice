...
function lockPage() {
    layer.open({
        ...
        '<div class="admin-header-lock-img"><img src="/statics/images/head.jpg"/></div>'+
...
$(document).ready(function () {
    layui.config({dir:'/statics/'})
    //默认显示菜单
    createMenu("/sys/menu/getMenuSide");
    setMainHeight();
});
        ...
        if (menuList[i].type === 1) {
            ...
            if (menuList[i].children) {
                ...
                for (var j = 0; j < menuList[i].children.length; j++) {
                    $li.find(".layui-nav-child").append(' <dd><a class="cy-page" href="javascript:;" data-url="' + menuList[i].children[j].url + '" title="' + menuList[i].children[j].name + '">' + menuList[i].children[j].name + '</a></dd>');
                }
            }else{
                var $li =$(_li);
                $li.find("a").after('<dl class="layui-nav-child">');
...
$("#menuSearch").keyup(function () {
    if ($("#menuSearch").val() == "") {
        ...
        for (var i = 0; i < menuList.length; i++) {
            ...
            if (menuList[i].children) {
                for (var j = 0; j < menuList[i].children.length; j++) {
                    if (menuList[i].children[j].type === 2) {
                        if (patt.test(menuList[i].children[j].name) || patt.test(menuList[i].children[j].url)) {
                            arr.push({
                                name: menuList[i].children[j].name,
                                url: menuList[i].children[j].url,
