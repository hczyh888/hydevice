...
public class BadManageController extends BaseController {
    BadManageService badManageService = BadManageService.me;
    public void index(){
        Page<BadManage> badManageTreeData = badManageService.badManageList(1, 1,"","id","desc");
        setAttr("badManageTreeData",badManageTreeData);
        render("badManage.html");
    }
    ...
    public void getList(){
        ...
        String[] paraStr = Query.makePara(new BadManage(),paraMap);
        String sWhere=paraStr[0];
        Page<BadManage> page= badManageService.badManageList(query.getPage(),query.getLimit(),sWhere,query.getSidx(),query.getOrder());

        List<BadManage> rows = page.getList();
     ...
     */
    public void edit(){
        BadManage badManage= badManageService.edit(getParaToInt(0,0));

        setAttr("badManage",badManage);
        render("edit.html");
    }
    /**
     * 更新
     */
    public void update(){
        BadManage badManage =getModel(BadManage.class,"badManage");
        Ret ret;
        if(badManageService.update(badManage)){
     ...
     */
    public void doadd(){
        BadManage badManage= getBean(BadManage.class,"badManage");
        Ret ret=null;
        if(badManage.save()){
            ret = Ret.ok("msg","保存成功！").set("code",0);
        }else{
    ...
    public void del(){
        ...
        if(badManageService.deleteById(id)){
